...
sed 's/\/\([0-9]\)\//\/0\1\//g' data.csv | sed 's/^\([0-9]\)\//0\1\//g' | sed 's/\//-/g' | sed 's/-\([0-9][0-9]\);/-20\1;/g' | sed 's/\;;/;\\N;/g' | sed 's/n/N/g' | sed 's/\;N;/;\\N;/g' | sed 's/\;N/;\\N/g' | sed 's/\;$/;\\N/g' | sed 's/[a-z]/\U&/g' | sed 's/,/./g' | sed 's/\([0-9][0-9]\)-\([0-9][0-9]\)-\([0-9][0-9][0-9][0-9]\);/\3-\2-\1;/g' | sed 's/;/,/g'